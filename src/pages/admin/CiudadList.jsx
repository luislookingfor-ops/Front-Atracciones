  ...
  const handleSave = async (e) => {
    ...
    try {
      if (currentItem.ciudadId) {
        await api.put(`/Ciudad/${currentItem.ciudadId}`, currentItem);
      } else {
        await api.post('/Ciudad', currentItem);
      }
            ...
            {filtered.map((item) => (
              <tr key={item.ciudadId} className="hover:bg-sand-50/50 transition-colors">
                ...
                <td className="px-6 py-4 text-sm text-sand-600">
                  {provinces.find(p => p.provinciaId === item.provinciaId)?.nombre || item.provinciaId}
                </td>
            ...
            <form onSubmit={handleSave} className="space-y-6">
              ...
              <div>
                ...
                <select 
                  ...
                  onChange={(e) => setCurrentItem({ ...currentItem, provinciaId: parseInt(e.target.value) })}
                >
              ...
              <div className="flex items-center gap-3">
                <input 
                  type="checkbox" id="city-estado"
                  checked={currentItem.estado}
                  onChange={(e) => setCurrentItem({ ...currentItem, estado: e.target.checked })}
                />
                <label htmlFor="city-estado" className="text-[10px] uppercase tracking-widest text-sand-500 font-medium cursor-pointer">Activa</label>
              </div>
