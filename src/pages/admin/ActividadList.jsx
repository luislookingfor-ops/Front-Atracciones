...
const ActividadList = () => {
  ...
  const [currentItem, setCurrentItem] = useState({ nombre: '', descripcion: '', estado: true });
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (currentItem.actividadId) {
        await api.put(`/Actividad/${currentItem.actividadId}`, currentItem);
      } else {
        await api.post('/Actividad', currentItem);
      }
      ...
      <div className="flex justify-between items-end">
        <div>
          <h2 style={{ fontFamily: 'Cormorant Garamond, serif' }} className="text-4xl font-light text-sand-950">
            Catálogo de <em className="italic">Actividades</em>
          </h2>
          <p className="text-sand-500 text-sm mt-2">Gestiona los tipos de experiencias disponibles.</p>
        </div>
        <button 
          onClick={() => { setCurrentItem({ nombre: '', descripcion: '', estado: true }); setIsModalOpen(true); }}
            ...
            {filtered.map((item) => (
              <tr key={item.actividadId} className="hover:bg-sand-50/50 transition-colors">
                <td className="px-6 py-4 font-medium text-sand-950 flex items-center gap-2">
                  <Zap className="w-3 h-3 text-ocean-600" /> {item.nombre}
                </td>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white max-w-md w-full p-8 border border-sand-200">
            <h3 style={{ fontFamily: 'Cormorant Garamond, serif' }} className="text-3xl font-light text-sand-950 mb-6">
              {currentItem.actividadId ? 'Editar' : 'Nueva'} <em className="italic">Actividad</em>
            </h3>
            <form onSubmit={handleSave} className="space-y-6">
              ...
              <div className="flex items-center gap-3">
                <input 
                  type="checkbox" id="act-estado"
                  checked={currentItem.estado}
                  onChange={(e) => setCurrentItem({ ...currentItem, estado: e.target.checked })}
                />
                <label htmlFor="act-estado" className="text-[10px] uppercase tracking-widest text-sand-500 font-medium cursor-pointer">Activa</label>
              </div>
